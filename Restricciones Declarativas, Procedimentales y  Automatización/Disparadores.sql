...
DECLARE
    ...
    fecha DATE;
BEGIN
    SELECT COUNT(idPelicula) INTO cantidad FROM Rentas WHERE :NEW.idCuenta = idCuenta AND :NEW.idPeliocula = idPelicula;
...
BEGIN
    ...
    IF v_pelicula IS NOT NULL THEN :NEW.idPelicula = v_pelicula;
    ELSE RAISE_APPLICATION_ERROR (-20001, 'Esta pelicula no esta en su catalogo.');
    END IF;
END;
/
...
BEGIN
    ...
    IF v_serie IS NOT NULL THEN :NEW.idSerie = v_serie;
    ELSE RAISE_APPLICATION_ERROR (-20001, 'Esta serie no esta en su catalogo.');
    END IF;
END;
/
